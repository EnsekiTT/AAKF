...
class DataReader(object):
    ...
    def make_mini_batch(self, mini_batch_size, length_of_sequence):
        ...
        answer = np.ndarray((mini_batch_size, length_of_sequence, self.datas.shape[1]+1), dtype=np.float32)

        for i in range(mini_batch_size):
            index = random.randint(0, len(self.datas) - length_of_sequence)
            sequences[i] = self.datas[index:index+length_of_sequence,:]
            answer[i] = np.c_[self.datas[index:index+length_of_sequence,:],
                              self.ans[index:index+length_of_sequence]]
